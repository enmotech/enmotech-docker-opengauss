...
# check password complexity, password should contains three kind letters of four letters at least
check_password_pass() {
  local password="$1"
  local match_times=0
  ...
  # check if password contains numeric letters
  if [[ "$password" =~ ^(.*[0-9]).*$ ]]; then
    match_times=$(($match_times+1))
  fi

  # check if password contains special letters
  if [[ "$password" =~ [^a-zA-Z0-9] ]]; then
    match_times=$(($match_times+1))
  fi

  if [[ "$password" =~ ^(.{8,}).*$ ]] && [[ "$match_times" -ge 3 ]]; then
    return 0
  fi
...
docker_verify_minimum_env() {
        ...
        if check_password_pass "$GS_PASSWORD"; then
                cat >&2 <<- 'EOWARN'
